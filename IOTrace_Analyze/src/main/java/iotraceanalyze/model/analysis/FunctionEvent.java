package iotraceanalyze.model.analysis;
...
import iotraceanalyze.model.analysis.file.FileLock;
...
public class FunctionEvent implements Comparable<FunctionEvent> {
    ...
    private void setRange(FileRange fileRange) {
        ...
        if (theSame != null) {
            for (FunctionEvent event : theSame) {
                event.fileRange = fileRange;
            }
        }
    }
    ...
    public void setRange(RangeType rangeType, String bytes, String offset) {
        setRange(rangeType, bytes, Long.parseLong(offset));
    }
