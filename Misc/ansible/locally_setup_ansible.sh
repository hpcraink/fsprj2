...
# 3. Install playbook dependencies & run it
cd /tmp/fsprj2-master/Misc/ansible/

sudo -u "${USERNAME}"  ansible-galaxy install -r requirements.yml

# NOTE: Must be run as root since we don't know the become-pw (and we also don't want to ask)
ansible localhost   -e hostname="${HOSTNAME}" -e username="${USERNAME}"  -m include_role -a name=roles/common

# Fix permissions  (despite `become: false`, we run ansbile as root --> hence permissions are wrong)
