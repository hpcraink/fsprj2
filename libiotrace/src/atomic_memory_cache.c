 ...
 * thread. Each array element holds a pointer to the first element of a linked
 * list of blocks. Each list holds only blocks of one size. The size of blocks
 * in a list is equal to the size given through the enum \a atomic_memory_size
 ...
 * \a atomic_memory_tls_free_start. Each array element holds a pointer to the
 * last element of the list in \a atomic_memory_tls_free_start with the
 ...
 * \a atomic_memory_tls_free_start. Each array element holds the count of
 * blocks inside the list in \a atomic_memory_tls_free_start with the
 * corresponding indices.
 *
 * This cache is only manipulated by the current thread and has therefore no
 * concurrent accesses from other threads. So it's not necessary to guard it
...
static ATTRIBUTE_THREAD size_t free_count[ATOMIC_MEMORY_MAX_THREADS_PER_PROCESS][atomic_memory_size_count] =
		{ { 0 } };
/**
 * Global cache (per process) of usable free memory blocks. Each array element
 * holds a pointer to the first element of a linked list of blocks. Each list
 * holds only blocks of one size belonging to one thread. The size of blocks
 * in a list is equal to the size given through the enum \a atomic_memory_size
 * equal to the second index of the array element. The thread the blocks
 * belonging to is given through the \a thread_id equal to the first index of
 * the array element.
 *
...
/**
 * Global (per process) counter of active threads. Has to be incremented for
 * each thread which calls the function \c atomic_memory_alloc(). This enables
 * creating a new unique \a thread_id for storing the new created block in the
 * different caches.
 *
...
/**
 * Thread local unique id. This id has to be created during first call of
 * function \c atomic_memory_alloc() from a thread. After that the id can be
 * used to store created memory blocks in caches. For that each block stores a
 * copy of the id in the field \a _thread inside the block (see struct
 ...
 * and increments the pointer \a atomic_memory_buffer_pos by size via atomic
 * instruction. Initializes the so reserved memory as an memory block and
 * generates therefore a new \a thread_id (via atomic increment of
...
void atomic_memory_free(union atomic_memory_block_tag_ptr block) {
	...
	// TODO: use local cache directly if thread == thread_id
