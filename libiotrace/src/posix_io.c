...
enum stat_file_type get_stat_file_type(mode_t st_mode)
{
    ...
    case S_IFSOCK:
    	return socket_file;
    default:
        return unknown_stat_file_type;
    }
}

void get_fstatat_flags(const int flags, struct fstatat_flags *sf)
{
    sf->at_empty_path = flags & AT_EMPTY_PATH ? 1 : 0;
    sf->at_no_automount = flags & AT_NO_AUTOMOUNT ? 1 : 0;
    sf->at_symlink_nofollow = flags & AT_SYMLINK_NOFOLLOW ? 1 : 0;
}
...
int WRAP(fcntl)(int fd, int cmd, ...)
{
    ...
    default:
        ...
        LOG_WARN("Unknown cmd value in call to %s.", data.function_name);
        ret = -1;
    }
    va_end(ap);
