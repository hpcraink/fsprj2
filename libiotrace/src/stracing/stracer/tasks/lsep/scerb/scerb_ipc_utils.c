...
int sm_ipc_attach_create_smo(
        ...
        bool o_creat) {
/* Create smo */
    int smo_fd = RTN_VAL_WHEN_ERRNO( shm_open(smo_name, O_RDWR | ((o_creat) ? (O_CREAT) : (0)), S_IRUSR | S_IWUSR) );
    if (o_creat) {
...
/* Get actual length */
    ...
    RTN_VAL_WHEN_ERRNO( CALL_REAL_POSIX_SYNC(fstat)(smo_fd, &stat_info) );
