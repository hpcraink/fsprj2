...
#       endif /* WANT_OPENMP */
        ...
        for (int i = 0; i < max_iterations; i++) {
            ...
            MPI_CHECK(MPI_File_write(fh, &val, 1, MPI_INT, &status_write));

            int count_write;
            MPI_CHECK(MPI_Get_count(&status_write, MPI_INT, &count_write));
            if (1 != count_write) {
                ERROR_FATAL("count_write is wrong", EINVAL);
            }

            // read
            MPI_Status status_read;
            DEBUG(printf ("Rank:%d Thread:%d read from pos:%lld\n", comm_rank, thread_num, pos));
            MPI_CHECK(MPI_File_seek(fh, pos, MPI_SEEK_SET));
            int val_read;
            MPI_CHECK(MPI_File_read(fh, &val_read, 1, MPI_INT, &status_read));
            int count_read;
            MPI_CHECK(MPI_Get_count(&status_read, MPI_INT, &count_read));
            if (1 != count_read) {
                ERROR_FATAL("count_read is wrong", EINVAL);
            }

            if (val != val_read) {
                ERROR_FATAL("val is not equal", val_read);
            }
